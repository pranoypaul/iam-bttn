...
Now you should access CLI script: $ iam --help
Alternatively you can access CLI script via poetry run: $ poetry run iam --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_IAM_COMPLETE=source_bash iam)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_IAM_COMPLETE=source_zsh iam)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_IAM_COMPLETE=source_fish iam)' >> ~/.config/fish/completions/iam.fish
EOF