...
.. code-block:: bash
    ...
    Commands:
      serve  iam CLI serve command.
    $ iam serve --help
    Usage: iam serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name iam -p 8000:8000 iam:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/iam:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  iam
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``iam.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean iam image
      install          Install iam with poetry
      metrics          Run iam metrics checks
      unit-test        Run iam unit tests
      integration-test  Run iam integration tests
      docs             Build iam documentation
...
* RedisClient ``iam.app.utils.redis``
* AiohttpClient ``iam.app.utils.aiohttp_client``
...
.. literalinclude:: ../iam/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``iam.config`` submodule.
...
.. literalinclude:: ../iam/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``iam.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../iam/app/router.py
    :language: python
